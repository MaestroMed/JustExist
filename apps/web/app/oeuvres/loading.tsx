    ...
    <PageShell>
      <Container size="full" className="py-20 md:py-24">
        <div className="flex flex-col gap-6">
          <div className="skeleton-shimmer h-4 w-40 rounded-full bg-[var(--color-cream-100)]/10" />
          <div
        ...
        <div className="mt-16 flex gap-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="skeleton-shimmer h-8 w-20 rounded-full bg-[var(--color-cream-100)]/10"
            />
          ))}
        </div>
        <div className="mt-10">
          <SkeletonGrid count={6} aspect="4/5" cols={3} />
        </div>
      </Container>
    </PageShell>
  );
